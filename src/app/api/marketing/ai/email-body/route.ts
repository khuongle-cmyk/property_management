...
export async function POST(req: Request) {
  ...
  let body: { tenantId?: string; template?: string; audience?: string; topic?: string };
  try {
...
Template style: ${body.template ?? "newsletter"}.
Audience: ${body.audience ?? "leads"}.
Topic / offer: ${body.topic ?? "general update"}.
Tone: professional, warm, Nordic business style. No placeholder brackets; keep under 400 words.`;
